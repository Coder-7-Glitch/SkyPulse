...
const AddNote = ({ setAddNote, getData }) => {
  ...
  const gradients = [
    ...
    { name: "Blue", class: "from-text_Color to-blue-400" },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#1f1f1f1a] backdrop-blur-sm">
      <div className="min-h-[70vh] w-[80%] max-w-[800px] rounded-xl bg-[#1b1b1b] border border-gray-700 p-4 px-5 py-5">
        ...
        <div
          className={`mt-2 h-[350px] overflow-hidden rounded-md border-none pb-[44px] text-zinc-900 ${data.background}`}
        >
          <div ref={editorRef} className="h-full w-full p-2" />
        </div>

        <div className="mt-9">
          <h1 className="font-worksans text-2xl font-medium text-text_Color">
            Select Background
          </h1>
        ...
        <div className="mt-8 flex gap-3">
          <button className="rounded-md bg-text_Color px-5 py-2" onClick={addNote}>
            Save
          </button>
          <button
            className="rounded-md border-2 border-text_Color px-5 py-2 text-text_Color transition-all duration-150 hover:bg-text_Color hover:text-white"
